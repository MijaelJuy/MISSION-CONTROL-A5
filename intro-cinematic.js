/**
 * Anna OS — Intro: zoom GSAP → Three.js (`space-journey.js`) → transición Anna OS desktop.
 *
 * Entrada síncrona vía snippet inline en index.html (__AnnaIntroKickRequest).
 */
/// <reference lib="dom" />
...
/** Import dinámico acotado: si Three/CDN se cuelgan, la intro no puede quedar bloqueada en await. */
function importSpaceJourneyTimeout(ms) {
  const load = import("./space-journey.js");
  const timeout = new Promise(function (_, rej) {
    window.setTimeout(function () {
      rej(new Error("Timeout cargando Three / space-journey.js"));
    }, ms);
  });
...
async function bootstrap() {
  ...
  /** @type {ReturnType<import("./space-journey.js").createAnnaSpaceJourney> | null} */
  let journey = null;

  /** @type {undefined | typeof import("./space-journey.js").createAnnaSpaceJourney} */
  let createJourneyFn;
  ...
  const primePromise = (async function prime() {
    ...
    try {
      const mod = await importSpaceJourneyTimeout(10000);
      createJourneyFn = mod.createAnnaSpaceJourney;
    } catch (e) {
      console.warn(
        "[Anna intro] No se pudo cargar space-journey (Three). Modo degradado:",
        e
      );
  ...
  console.info(
    "[Anna intro] Lista. HTTPS + servidor recomendados (los módulos no funcionan bien con file://)."
  );
}
